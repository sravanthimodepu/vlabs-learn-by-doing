...
class SettingsRenderer extends SettingsLoadSave{
    ...
    protected function header($savePostTo){
        ...
        $PAGE->set_url($CFG->wwwroot."/theme/saga/settings/index.php");
        echo $OUTPUT->header();
    ...
    protected function thumbListRender($id, $default = null, $options = null, $classes = null, $tab){
        if($options != null && sizeof($options)>0){
            ...
            foreach ($options as $key => $value) {
                echo '<img class="thumbListItem'.(($value==$loadValue)?' selected':'').'" src="images/'.$key.'" value="'.$id.'_'.$value.'" alt="'.ucfirst($value).'" title="'.ucfirst($value).'"/>';
            }
    ...
    protected function slideshowRender($id, $default = null, $options = null, $classes = null, $tab){
        ...
        if((!is_array($items)) || (sizeof($items) == 0)){
            ...
            echo                        '<thead>';
            echo                            '<tr>';
            echo                                '<td align="center">Slider 1</td>'; 
            echo                            '</tr>';
            echo                        '</thead>';
            ...
            foreach ($items as $k => $v) {
                ...
                echo                        '<thead>';
                echo                            '<tr>';
                echo                                '<td align="center">Slider '.($n++).'</td>'; 
                echo                            '</tr>';
                echo                        '</thead>';
                echo                        '<tbody style="display: none;">';      
                echo                            '<tr>';
