...
** Introduction
Importing wiki and moodle database from LbD server to local machine (Base4 10.4.12.133).
** Source information
 - Balamma
** Detailed information of LbD server (production)
 ...
 - Credentials - Saved in =password.txt= file under systems team 
 - Location of CIT_VIDEOS in ftp server
...
** Detailed information of LbD server (local machine in Base4)
 ...
 - Copied wiki, moodle, lbdvideos, jmoodledata, pdf_wiki and CIT_VIDEOS to local machine
  + *wiki:* =/var/www/html/wiki=
  + *moodle:* =/var/www/html/moodle=
...
#+end_example
 - Copying wiki, moodle, jmoodledata, pdf_wiki, lbdvideos and CIT_VIDEOS folders from LbD server to local machine (Base4 container) using rsync
#+BEGIN_EXAMPLE
...
#+END_EXAMPLE
** Chaning domain name from http://enhanceedu.iiit.ac.in to lbd.vlabs.ac.in in local machine (Base4)
